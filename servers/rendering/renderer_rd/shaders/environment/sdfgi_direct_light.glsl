...
void compute_area_light(uint index, vec3 position, out float attenuation, out vec3 light_vec, out float light_distance, out vec3 texture_color) {
	...
	light_vec = normalize(light_rel_vec);
	...
	vec3 light_points[4];
	light_points[0] = lights.data[index].position - position;
	light_points[1] = lights.data[index].position + area_width - position;
	light_points[2] = lights.data[index].position + area_width + area_height - position;
	light_points[3] = lights.data[index].position + area_height - position;
	float solid_angle = quad_solid_angle(light_points);
	float cosine = max(0.0, dot(area_direction, position - lights.data[index].position)); // makes light only effective in front
	...
	if (lights.data[index].area_projector_rect != vec4(0.0)) {
		float area = a_len * b_len;
		float lod = light_distance / sqrt(area);
		lod = log(2048.0 * lod) / log(3.0);

		vec2 uv = (closest_point_local_to_light.xy + vec2(a_half_len, b_half_len)) / vec2(a_len, b_len);
		texture_color = fetch_ltc_lod(vec2(1.0) - uv, lights.data[index].area_projector_rect, lod, max_mipmap, area_light_atlas, linear_sampler);
	} else {
		texture_color = vec3(1.0); // required for some reason...
	}
}
